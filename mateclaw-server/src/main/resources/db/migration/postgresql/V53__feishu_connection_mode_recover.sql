...
-- See h2/V53 for the full rationale. Same surgery, PostgreSQL dialect
-- (LOCATE -> POSITION .. IN, SUBSTRING(x, n) -> SUBSTR(x, n)).
...
SET config_json = CASE
        ...
        WHEN POSITION('"connection_mode"' IN config_json) = 0 THEN
            CONCAT('{"connection_mode":"websocket",', SUBSTR(config_json, 2))
        ELSE
            REPLACE(
                REPLACE(config_json,
