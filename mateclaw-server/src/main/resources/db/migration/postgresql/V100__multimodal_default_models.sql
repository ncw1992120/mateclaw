...
VALUES
  (1000002001, 'default.vision_model', '', 'Default vision-capable model id (mate_model_config.id) used by sidecar router when primary model lacks VISION modality', NOW(), NOW())
ON CONFLICT (id) DO NOTHING;

INSERT INTO mate_system_setting (id, setting_key, setting_value, description, create_time, update_time)
VALUES
  (1000002002, 'default.video_model', '', 'Default video-capable model id (mate_model_config.id) used by sidecar router when primary model lacks VIDEO modality', NOW(), NOW())
ON CONFLICT (id) DO NOTHING;
