  ...
  <el-dialog v-model="ui.preview.visible" :title="title" width="780px" :close-on-click-modal="false">
    <el-tabs v-model="tab">
      ...
      <el-tab-pane label="处理日志" name="log">
        <div v-for="l in payload.logs" :key="l.time" class="log-line">
...
const { state, getPreviewPayload } = useInsight()
const ui = state.ui
const tab = ref('data')
const page = ref(1)
...
const payload = computed(() => getPreviewPayload())

// 分页：按当前页切片
const pagedRows = computed(() => {
  const start = (page.value - 1) * pageSize.value
  return payload.value.dataRows.slice(start, start + pageSize.value)
})

// 打开预览时，回到指定 Tab（来自「筛选预览」时强制数据预览）并重置分页
watch(
  () => ui.preview.visible,
  (v) => {
    if (v) {
