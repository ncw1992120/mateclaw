<template>
  <div class="ai-analysis-widget">
    <div class="analysis-header" :class="`title-bar-${props.component.titleBarStyle ?? 'standard'}`">
      <span v-if="showTitle !== false && component.titleBarStyle !== 'hidden'" class="analysis-title"><DashboardComponentIcon type="aiAnalysis" :dashboard-theme="dashboardTheme" :title-icon-style="titleIconStylePreview ?? component.titleIconStyle" />{{ component.title }}</span>
      <el-button
        v-if="!generating"
        type="primary"
        size="small"
