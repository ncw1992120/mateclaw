...
export function defaultComponentVisualStyle(type: InsightComponentType): ComponentVisualStyle {
  ...
  return {
    ...
    radius: 12,
    shadow: type === 'filter' || type === 'timeFilter' ? 'none' : 'subtle',
    padding: 0,
  }
}
...
export function normalizeComponentVisualStyle(
  ...
  const legacyBorder = style && 'border' in style ? style.border : undefined
  ...
  return {
    ...
    border: {
      ...
      colorMode: (style?.border as ComponentVisualStyle['border'] | undefined)?.colorMode ?? (borderColor ? 'custom' : 'theme'),
