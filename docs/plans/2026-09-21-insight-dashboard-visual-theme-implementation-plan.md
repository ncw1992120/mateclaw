...
- [ ] 写失败测试：入口可键盘操作；预设切换立刻改变画布、未点保存时后端不会收到更新；存在主题级覆盖时切换预设必须确认；保存后刷新仍是所选主题；恢复预设只清覆盖项；切换页面/组件、打开侧栏后主题不串到编辑器外壳；色值非法或对比不足时有就地提示并禁用保存该配置；主题变更进入既有撤销/重做历史。
- [ ] 运行面板和编辑器相关 Vitest 测试，确认新行为失败。
- [ ] 实现 10 套缩略预设、主色/背景/卡片/文本/语义色、指标色板、图表色板及形状配置。给取色器预设接入统一 Token；自定义项展示当前生效来源；主题面板的实时预览不发数据查询，保存沿用仪表盘既有按钮。抽屉关闭后继续保留未保存草稿，与编辑器其他配置一致。
- [ ] 为预设卡补齐 `role=radio`、Enter/Space、方向键和焦点回落；切换预设若会清覆盖项使用二次确认；保存中禁用重复保存，保存失败就地提示且不丢草稿。
- [ ] 复跑定向测试、`pnpm build`；检查 375/768/1024/1440 宽度抽屉不溢出，Tab 和焦点可达。
- [ ] 精确暂存并提交 `feat(洞察): 增加仪表盘主题外观配置`。
