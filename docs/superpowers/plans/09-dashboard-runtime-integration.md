...
本节及统一验收记录中的最新记录优先于本文件前面的历史复现段落：双源 `1158 pixels (ratio 0.01)` 快照差异已关闭；`22 passed` 是历史基线，当前本地模拟 Chrome channel 完整矩阵已更新为 `26 passed (1.4m)`，并包含指标平台分页与主入口输入语义回归。UI 单测基线为 `12 files / 44 tests passed`，DataAgent surefire 汇总为 `158 tests / 0 failures / 0 errors / 0 skipped`。仍未关闭的只有真实 Aloudata 结果授权（`SM_02_0038`）、正式外部存储/API、全站跨浏览器可访问性和权限专项。
