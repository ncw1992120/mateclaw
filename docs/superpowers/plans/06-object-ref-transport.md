...
本轮关联结果（2026-09-13）：VIS-UI04 的大结果页面只展示 10 行截断表格，没有明确 `outputRef`/受限预览边界提示，保持 `FAIL`；VIS-UI08 的双源 Table 在仪表盘预览中显示空白，保持 `FAIL`。

**状态更正（2026-09-14）：** 大结果页面已增加 `outputRef` 与受限预览边界提示，双源 Table 可视高度也已修复；本地模拟 E2E 与 Chrome CDP 已复验关闭上述两个历史问题。真实对象存储替换仍按外部环境条件保留。
