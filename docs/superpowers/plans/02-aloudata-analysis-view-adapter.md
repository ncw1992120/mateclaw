...
**真实只读复验补充（2026-09-14）：** 当前认证上下文对 `Demo_view` 的目录/详情请求均为 `code=200`，详情包含 5 个指标、3 个维度；语义层 5 行基线查询及目录前 20 个视图探测均返回 `SM_02_0038`。ALO-X02 仍需 Aloudata 提供至少一个可查询视图后再验证筛选下推。
