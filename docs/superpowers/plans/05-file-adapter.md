...
本轮关联结果（2026-09-13）：VIS-UI02 的文件输入预览能返回字段和数据，但 Descriptor 仍显示 `0 个字段`，并且预览 JSON 横向溢出，保持 `FAIL`；VIS-UI08 的 HTTP/API+文件 Table 预览可视高度为 `0`，保持 `FAIL`。

**状态更正（2026-09-14）：** Descriptor 异步字段摘要、预览布局溢出和 Table 可视高度均已修复；文件输入与 HTTP/API+文件本地模拟页面已通过字段/数据/结果可见性复验。上述 `FAIL` 仅保留为历史复现，当前本地 VIS-UI02/VIS-UI08 已关闭。
