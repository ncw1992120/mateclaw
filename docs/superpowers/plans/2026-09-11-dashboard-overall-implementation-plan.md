...
**产品闭环状态更正（2026-09-14，当前工作树）：** 本地模拟环境的正式前端入口、文件上传/统一预览、脚本结果绑定、双源运行时、错误态、Descriptor 字段可见性和数据源表单主题适配已通过真实 Chrome channel Playwright/CDP 验证；G3 在“本地模拟”范围为 `PASS`。UI 单测当前为 `12 files / 44 tests passed`，并已修复 Chrome autofill 主题覆盖、Playwright channel 配置覆盖、预览空态测试误假设、数据表键盘操作、空表目录刷新、API/JDBC 来源边界、AI 助手关闭按钮和页面树操作按钮可访问名称。真实 Aloudata 授权、正式对象存储替换、非 Chrome 原生控件外观、其他历史页面主题和跨工作区权限矩阵仍是外部或后续 Gate，不能据此宣称生产环境完成。
...
**2026-09-15 键盘事件隔离补充：** 列表卡片和数据源条目保留父级 Enter/Space 语义，同时对内部操作区增加 `keydown.stop`，避免编辑/删除按钮的键盘操作冒泡为预览/选择。新增回归用例修复前失败、修复后通过；产品入口 E2E `20 passed`、UI 单测 `13 files / 46 tests passed`、生产构建均已复验。该修复属于本地模拟前端闭环，不改变真实 Aloudata、对象存储和权限 Gate。
...
**2026-09-15 工作空间二级菜单语义补充：** 无 `href` 的工作空间子菜单改为可键盘操作的 `tablist/tab`，补齐选中态、roving focus 及 Enter/Space/方向键/Home/End；定向 Chrome E2E 和 CDP 现场通过，5 个子菜单实际焦点切换正常。全站跨浏览器审计仍保持后续专项。

**2026-09-15 问数数据源浏览入口语义补充：** 数据源下拉“浏览”入口补齐按钮语义、动态名称和 Enter/Space 操作，并隔离事件冒泡；定向 Chrome E2E、UI 单测、构建和 CDP 实际抽屉打开均通过。该修复不改变数据源勾选和浏览接口契约。
...
**2026-09-15 最新本地验收基线：** 当前工作树使用同一轮模拟 Compose、工作区 `1`、JWT、`MATECLAW_E2E_ALOUDATA_MODE=simulation` 和 `MATECLAW_UI_BASE_URL=http://127.0.0.1:15174` 执行完整 Chrome channel E2E，结果为 `29 passed (4.5m)`、无跳过；UI 单测为 `16 files / 49 tests passed`，生产构建成功，`DESIGN-PASS` 通过。该基线包含帮助中心、导出按钮、属性面板、列表/菜单和问数数据源浏览入口的键盘语义修复；真实 Aloudata 结果授权、正式外部服务及权限专项仍不因本地模拟结果关闭。
...
**2026-09-15 数据源表单回归修复：** 数据源连接表单原生输入和复选框已补齐显式 AX 名称，并修复复选框 CSS 多余括号导致的 Vite 开发服务 HTTP 500。定向 E2E `1 passed`、完整 Chrome channel 矩阵 `25 passed (1.5m)`、UI 单测 `12 files / 44 tests passed` 和生产构建均通过；本地 Chrome CDP 复验 5 个复选框名称齐全且 AX 空名称数为 `0`。
...
**当前权威基线（2026-09-15）：** 前文保留的历史失败段落只用于追溯，不代表当前状态。`22 passed`、`25 passed`、`26 passed`、`28 passed` 是历史基线；最新本地模拟 Chrome channel 完整矩阵为 `29 passed (4.5m)`，双源最终结果实际为 `5` 行且包含 `120.5`，严格快照通过；UI 单测为 `16 files / 49 tests passed`，生产构建成功。当前仅保留真实 Aloudata 结果授权、正式外部 API/对象存储、全站跨浏览器可访问性和权限专项 Gate。
...
**2026-09-15 扩展键盘巡检：** Chrome CDP 对数据集列表/新建、配置中心四个子页及既有五个主入口执行 120～160 次 Tab，当前可见交互控件均无空 AX 名称。该证据只关闭“可见控件命名缺失”这一子风险，完整键盘顺序、非 Chrome 原生控件和跨浏览器审计继续保持 PARTIAL。
...
**2026-09-15 入口矩阵复验：** 使用用户 Chrome 保存的 JWT/工作区上下文执行 `dataset-management-entry.spec.ts` 全量 `27 passed (1.4m)`，包含图表多 Tab fixture；双源专用矩阵本轮因缺少 seed Dashboard ID 按 fail-fast 规则停止，不将状态变量缺失记为业务失败。
