...
| 数据连接管理 | `PARTIAL` | 新建已先选择 MySQL/PostgreSQL/SQL Server/Aloudata 类型；列表仍需补齐 HTTP/API 与文件登记管理 |
| 五种来源数据集创建 | `PARTIAL` | 已有 `/datasets`、`/datasets/new`、`/datasets/:id/edit` 和配置中心入口；上传后预览及完整前端创建 E2E 仍待补齐 |
