...
> **注意**：本文档与项目根目录 `prompts/数据分析助手.md` 描述同一套工作流程。当两处存在差异时，以 `数据分析助手.md` 为权威主文件。本文档侧重技能注册和工具声明，完整流程细节见主文件。
>
...
9. **系统自动保障**：queryResultType 已强制为 DATA；timeConstraint 会自动规范化（补括号、BETWEEN转AND、补方括号）；查询请求会自动校验（中文展示名、同环比约束、维度可用性等）
