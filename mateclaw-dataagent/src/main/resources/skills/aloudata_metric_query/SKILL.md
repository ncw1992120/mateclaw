...
version: "1.0.0"
description: "Aloudata 指标平台查询的唯一入口。凡涉及指标数值、趋势、对比、排名、占比、按维度拆解的问数请求，必须先 load_skill('aloudata_metric_query') 加载完整工作流，再按流程执行，禁止跳过流程直接调用 aloudata_metrics_query。硬约束：禁止凭猜测构造 metricName/dimName，必须先 aloudata_search_semantic 检索获得英文名；timeConstraint 仅支持 metric_time 维度且必须用 () 包裹；同环比须 metric_time 单值锚点；涉及相对时间（昨日/上周几/近N天等）必须先调 DateTimeTool.getCurrentDate 获取当前日期，禁止主观推算。当用户问题与指标名称完全匹配（=metricName或展示名，且匹配名完整覆盖原话限定词）时，直接查询、禁止再做消歧确认；用户未指定维度时，dimensions 使用默认指标时间维度（metric_time，如 metric_time__month），不得追问维度或凭空选用其他维度。keyword 必须原样保留原话限定词、禁止截断或精简（不确定完整指标名时直接传用户原话整句）；「✅ 指标名称精确匹配」须核对匹配展示名完整覆盖原话限定词，缺失时禁止直接采信。"
dependencies:
  tools:
...
**时间解析硬约束（先取真实日期，禁止主观推算）**：只要时间要素包含相对表述——"今天、昨天、前天、本周/上周几、本月、上月、近N天、今年、去年同期"等——必须**先调用内置工具 getCurrentDate（DateTimeTool.getCurrentDate）获取服务器当前日期与星期**，再以其返回值为唯一锚点换算出具体日期/区间/timeConstraint 表达式；**严禁凭主观印象推算今天日期或星期几，严禁依赖训练记忆里的时间**。`aloudata_search_semantic` 返回头部注入的「当前日期」锚点同样可信，可作为同轮换算依据。换算完成后必须**自检**：结果日期的星期与表述一致（如"上周五"换算结果必须为星期五）、区间长度与表述一致（如"近7天"为7个自然日），自检不符时重新换算。timeConstraint 中的 Today() 由查询引擎执行时求值可放心使用；但"上周三""昨天"等必须先知道当天日期才能确定的表述，只能以 getCurrentDate 的结果换算。需要精确到时分秒或判断跨天边界时改用 getCurrentDateTime。
...
**keyword 必须原样保留用户标点（硬性）**：用户问句带括号/连接符/下划线等标点时（如「销售金额（含税）」「高级-VIP 客户」），keyword 必须**原样传递**这些标点（尊重用户的全角/半角写法），**禁止删除、改写、转换全角半角或截断关键词**。指标匹配对标点差异不敏感——全角「（含税）」与半角「(含税)」、带横线/下划线与不带，都能匹配到同一指标，改写反而可能引入偏差。若少传了标点/限定词，系统会在检索侧用用户原话自动做标点还原与精确匹配兜底，但不要依赖兜底、尽量一次传对。

**keyword 宁整勿截（硬性，防短名劫持）**：keyword 使用用户原话中**完整的指标名短语**，包含全部限定词、括号内容与连接符（如「交易市占率（整体）」「期货收入-应收-当年」）；不确定哪部分是完整指标名时，**直接把用户原话整句作为 keyword 传入**，宁可多检索也不截断/精简——截断到同族短名（如「销售金额（含税）」→「销售金额」）可能被「✅ 指标名称精确匹配」锁死成库里的短名指标，造成错查而非多问。检索返回「✅ 指标名称精确匹配」时，同样先核对匹配展示名是否完整覆盖原话限定词，缺失时禁止直接采信（见第三点五步）。
...
5. **用户问题与指标名称完全匹配**：用户表述 = 某指标的 metricName 或展示名（`aloudata_search_semantic` 返回「✅ 指标名称精确匹配」时）→ 直接使用该指标构造查询，**禁止再做消歧确认**。**复核前提**：匹配展示名完整覆盖用户原话限定词与括号内容；若匹配名缺少原话限定词（如匹配「销售金额」而原话是「销售金额（含税）」），禁止直接采信，改用完整原话重新检索或向用户确认（见第三步）
...
1. **重试上限：同一错误连续出现 ≥5 次时，才禁止继续变换参数重试**。指标查询失败大多是参数构造导致的——每次失败后应对照第六步输出自检清单定位并修正具体参数（orders 形态、维度可用性、同环比约束、timeConstraint/filters 格式等），有目的地修正后重试，最多可重试至同一错误连续出现 5 次；禁止无依据的盲目试错（如"去掉 orders 试试""换个时间范围试试"）。若修正后同一错误仍连续出现 5 次，说明大概率非参数问题：应停止查询，检查数据源查询通道（指标平台「指标应用 → API集成」中的服务地址、tenant-id、认证值）或查询引擎状态，无法确认时如实向用户反馈并等待指示。
...
10. **失败止损**：同一错误连续出现 ≥5 次即停止变换参数重试；此前的每次重试都必须依据第六步自检清单修正具体参数（失败大多是参数构造导致），禁止盲目随机变换（见"查询失败处理与止损"）
11. **指标名称完全匹配即直查（须先核对限定词覆盖）**：用户问题与某指标 metricName/展示名完全匹配（`aloudata_search_semantic` 返回「✅ 指标名称精确匹配」）时，直接构造查询，**禁止再次向用户做消歧确认**；**前提**：匹配展示名完整覆盖用户原话中的限定词与括号内容，若匹配名缺少原话限定词（如匹配「销售金额」而原话是「销售金额（含税）」），禁止直接采信，改用完整原话重新检索或向用户确认（见第三点五步）
...
14. **对标点不敏感，保留原话标点（硬性）**：指标匹配对标点差异不敏感（全角/半角括号、-、_、空白等不影响匹配）。`aloudata_search_semantic` 的 keyword 必须**原样保留**用户原话中的标点，禁止删除、改写、转换全角半角或截断；若需精简只能去掉与指标无关的修饰词，括号内的限定词（如（含税）（整体）（汇总））及其标点必须完整保留（见第三步）
15. **相对时间先取真实日期（硬性）**：时间要素含相对表述（"昨日/上周几/近N天/去年同期"等）时，必须先调用内置工具 getCurrentDate 获取服务器当前日期与星期，以其为唯一锚点换算 timeConstraint 与对比区间；`aloudata_search_semantic` 返回头部已附「日期对照表」（本周/上周逐日、本月/上月/去年同月、近N天区间），换算一律**查表取日期**，禁止凭主观印象或自行推算当前日期与星期（见第一步"时间解析硬约束"）；换算后自检结果星期与区间长度同表述一致
15b. **系统时间锁定优先（硬性）**：`aloudata_metrics_query` 返回头部若出现「**系统时间锁定**：…，timeConstraint 已由系统据此设置，请勿修改」，说明系统已按原话相对时间（昨日/上周几/近N天/上月/去年同期等）确定性锁定时间区间，**必须直接采用查询结果作答，禁止再修改 timeConstraint 或重试**；该机制由服务端规则解析覆盖 LLM 传入的时间表达式，可避免星期/日期推算错误（见第 7 条相关修复）
16. **keyword 宁整勿截，精确匹配先复核（硬性）**：keyword 使用原话完整指标名短语（含限定词与括号内容），不确定时直接传原话整句；截断到同族短名可能被「✅ 指标名称精确匹配」锁死成错误指标。收到精确匹配提示时先核对匹配展示名是否完整覆盖原话限定词，缺失时禁止直接采信、改用完整原话重新检索（见第三步）
