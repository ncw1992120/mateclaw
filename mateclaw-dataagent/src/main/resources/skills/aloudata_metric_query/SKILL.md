...
**时间解析硬约束（先取真实日期，禁止主观推算）**：只要时间要素包含相对表述——"今天、昨天、前天、本周/上周几、本月、上月、近N天、今年、去年同期"等——必须**先调用内置工具 getCurrentDate（DateTimeTool.getCurrentDate）获取服务器当前日期与星期**，再以其返回值为唯一锚点换算出具体日期/区间/timeConstraint 表达式；**严禁凭主观印象推算今天日期或星期几，严禁依赖训练记忆里的时间**。timeConstraint 中的 Today() 由查询引擎执行时求值可放心使用；但"上周三""昨天"等必须先知道当天日期才能确定的表述，只能以 getCurrentDate 的结果换算。需要精确到时分秒或判断跨天边界时改用 getCurrentDateTime。
...
15. **相对时间先取真实日期（硬性）**：时间要素含相对表述（"昨日/上周几/近N天/去年同期"等）时，必须先调用内置工具 getCurrentDate 获取服务器当前日期与星期，以其为唯一锚点换算 timeConstraint 与对比区间；禁止凭主观印象推算当前日期与星期（见第一步"时间解析硬约束"）
