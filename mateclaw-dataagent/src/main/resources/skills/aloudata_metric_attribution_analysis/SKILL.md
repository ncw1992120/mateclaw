...
**keyword 宁整勿截（硬性，防短名劫持）**：keyword 使用原话**完整的指标名短语**，完整保留限定词与括号内容（如「销售总金额（华北）」（华北）），禁止截断或精简；不确定完整指标名时直接把用户原话整句作为 keyword 传入。检索返回「✅ 指标名称精确匹配」时，先核对匹配展示名是否完整覆盖原话限定词，缺失时禁止直接采信、改用完整原话重新检索（同 `aloudata_metric_query` 第三步）。
