...
| **mateclaw-dataagent** | 后端主体：问数编排、语义层、指标平台接入、工具、洞察仪表盘、权限治理（独立端口 `18089`，上下文路径 `/`） | Spring Boot 3 + MyBatis-Plus + ES |
...
> **同步黑名单过滤**：元数据入库前经 `AloudataSyncFilterSupport`（`support` 包）做 QLExpress 动态黑名单判断——命中任一配置表达式的类目/指标/维度不落库持久化，旧版本数据随版本清理自动移除。规则两级配置、合并生效：全局系统配置 `aloudata.sync.filter.expressions`（表达式 JSON 数组）与数据源级 `connection_params.syncFilterExpressions`（数据源编辑表单"同步过滤规则"，每行一条）。同一份表达式会在类目/指标/维度三处分别求值：类目上下文里 `categoryName/name` 是类目自身名称，另有 `categoryType`（`CATEGORY_METRIC`/`CATEGORY_DIMENSION`）可用；指标/维度上下文里 `categoryName` 表示"所属类目的名字"。**注意：指标类目树与维度类目树常使用同一套业务域命名，按名称写的规则会同时命中两棵树，只想过滤其中一侧需用 `categoryType` 显式限定**（如 `categoryType == 'CATEGORY_DIMENSION' && categoryName in ('测试类目')`）。类目命中后仅在其所在类目树内级联子类目（父子关系只按 `parentId` 推导，不跨树）。表达式执行异常按未命中处理（fail-open），避免坏规则阻断同步。全零保护按类型分别校验：本次指标（或维度）拉取为 0 且库中已有存量时中止同步并保留旧数据，避免过滤规则过宽或接口异常导致误清空。
...
启用企业认证后（`mateclaw.pilot.enabled=true`），登录页出现**三种入口**：开机账号登录（AD）、UM 账号登录（UM）、本地账号登录（仅白名单账号）。口令校验委托给平安领航服务（**账密代验**：无浏览器跳转，DataAgent 将口令转发领航校验），认证通过后**自动落影子账号并签发与本地登录完全一致的 JWT**，下游零感知。
...
sequenceDiagram
    participant U as 用户
    participant FE as 登录页（DataAgent-UI）
    participant DA as DataAgent（18089）
    participant PL as 领航认证服务
    U->>FE: 输入账号口令（开机账号 / UM 账号 / 本地账号）
    FE->>DA: GET /v1/auth/pubkey（拉取 RSA 公钥）
    DA-->>FE: 公钥 PEM（公开）
    FE->>FE: jsencrypt RSA-OAEP 加密信封 ts:口令
    FE->>DA: POST /v1/auth/login（口令密文 · authnType · channel?）
    DA->>DA: 解密 + 时间戳防重放校验（±5 分钟窗口）
    alt 企业通道（非白名单账号，领航代验）
        DA->>PL: POST /v3/assertion（账密代验）
        alt 风控 NEED_RAND_CODE
            PL-->>DA: message=NEED_RAND_CODE
            DA-->>FE: HTTP 429（需要图形验证码）
            FE->>DA: GET /v1/auth/captcha?authnType=
            DA->>PL: POST /v3/assertion/captcha（authnMechanism）
            PL-->>DA: requestId + captchaImage（base64）
            DA-->>FE: 验证码图片
            U->>FE: 输入图形验证码
            FE->>DA: 重试登录（携带 requestId + validCode）
            DA->>PL: POST /v3/assertion（additionalInfo 带验证码）
            PL-->>DA: WRONG_IMAGE_CODE → 401（前端刷新验证码）
        end
        PL-->>DA: code=0 认证通过
    else 本地通道（channel=local / 白名单账号如 admin）
        DA->>DA: 本地密码校验（AuthService 兜底，防企业侧故障锁死管理员）
    end
    DA->>DA: 影子账号（随机口令，不可本地登录）
    DA->>DA: 签发本地 JWT（与本地登录完全一致）
    DA-->>FE: token + 用户信息 + 工作区列表
```
...
```mermaid
sequenceDiagram
    participant B as 浏览器（*.paic.com.cn 域）
    participant FE as 登录页
    participant DA as DataAgent
    participant PL as 领航认证服务
    B->>FE: 加载登录页（携带共享域 SSO Cookie）
    FE->>DA: GET /v1/auth/mode
    DA-->>FE: provider=pilot · ssoCookieName=CAS_SSO_COOKIE
    FE->>B: 读取 SSO Cookie（非 HttpOnly，JS 可读）
    alt 命中 Cookie
        FE->>DA: POST /v1/auth/sso/login（RSA 加密信封 ts:cookie）
        DA->>PL: POST /v3/assertion/sso（ssoCookie · ssoType=CAS_SSO_COOKIE）
        PL-->>DA: code=0 · isSuccess=true · userName
        DA->>DA: 影子账号 + 签发 JWT
        DA-->>FE: 免登成功，静默进入工作台
    else 无 Cookie / 校验失败
        DA-->>FE: 401（静默降级，不打断用户）
        FE->>U: 展示普通登录表单
    end
    Note over FE,PL: 会话期间：POST /v1/auth/sso/renewal（需登录）→ /v3/sso/renewal 续期企业侧会话，失败仅告警、不影响本地 JWT
```

**传输加密与防重放**（无 TLS 部署的兜底，防被动窃听）：

- 每次提交敏感字段（口令 / SSO Cookie / 改密口令）前，先从 `GET /v1/auth/pubkey` 拉取 RSA 公钥，用 jsencrypt 做 **RSA-OAEP** 加密；信封为 `base64(RSA-OAEP(base64(UTF-8("毫秒时间戳:明文"))))`——明文先做 UTF-8→Base64 进 OAEP（jsencrypt 对非 ASCII 字符的 OAEP 处理有缺陷），后端 `TransportCryptoService` 解密还原后校验 **±5 分钟**时间窗，超窗拒绝（防重放）。
- 私钥仅存服务端（`MATECLAW_AUTH_CRYPTO_PRIVATE_KEY`，多实例必须共享同一私钥），解密明文不落日志；公钥公开可下发。
- 局限：只防被动窃听，页面若经 HTTP 加载无法防主动 MITM 篡改页面——生产仍应以 HTTPS 为准。

**失败处理与审计**：

- 按用户名限速：**10 次/5 分钟**窗口内仅凭据类失败（401）计数，超限返回 403；验证码流程（429）与基础设施故障（502）不计数，登录成功即清零。
- 凭据错误统一文案"用户名或密码错误"，不泄露领航侧 message，防账号探测。
- 成功登录写审计日志：`channel=[LOCAL | PILOT_UM | PILOT_AD | PILOT_SSO]`，标明实际生效的认证通道。
...
- **独立 Spring Boot 应用**：端口 `18089`，上下文路径 `/`，API 文档地址 `/v1/swagger-ui.html`，自带 Swagger、Actuator 健康监控。
- **依赖**：
