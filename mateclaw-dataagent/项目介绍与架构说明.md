...
> **同步黑名单过滤**：元数据入库前经 `AloudataSyncFilterSupport`（`support` 包）做 QLExpress 动态黑名单判断——命中任一配置表达式（或其所属类目命中，类目级联子类目）的类目/指标/维度不落库持久化，旧版本数据随版本清理自动移除。规则两级配置、合并生效：全局系统配置 `aloudata.sync.filter.expressions`（表达式 JSON 数组）与数据源级 `connection_params.syncFilterExpressions`（数据源编辑表单"同步过滤规则"，每行一条）。表达式执行异常按未命中处理（fail-open），避免坏规则阻断同步。
